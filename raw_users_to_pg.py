...
def add_new_user()-> None:
    ...
    и добавляет запись в таблицу raw.raw_users в развернутом PostgreSQL
...
def update_info_about_current_user()-> None:
    """Обновляет информацию о случайно выбранном из базы пользователе.
    ...
    Поле updated_at устанавливается на текущую дату и время. Для обновления 
    значений полей используется библиотека Faker.
    ...
    try:
        ...
        columns_to_update = ["last_name", "email"]
        num_fields_to_update = random.randint(1, 2)
        selected_fields = random.sample(columns_to_update, num_fields_to_update)
